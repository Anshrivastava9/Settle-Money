...
onload = function () {
    ...
    function createData() {
         var x = JSON.parse(sessionStorage.getItem("Tmembers"));
        const sz = x//Math.floor(Math.random() * 8) + 2;
        ...
        //const edges = [];
        var str__note = localStorage.getItem("note");
        
        const NobJ = JSON.parse(str__note);
        NobJ.forEach(function(element) {
            edges.push({ from: element.from, to: element.to, label: element.money});
        });
        // edges.push({ from: 4, to: 1, label: "90" });
        // edges.push({ from: 2, to: 1, label: "80" });
        // edges.push({ from: 3, to: 2, label: "70" });
        // edges.push({ from: 1, to: 4, label: "270" });
        // edges.push({ from: 2, to: 3, label: "170" });
